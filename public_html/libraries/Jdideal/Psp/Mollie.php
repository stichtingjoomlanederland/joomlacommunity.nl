...
class Mollie
{
	...
	public function getAvailablePaymentMethods(): array
	{
		return [
			...
			'creditcard'   => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_CREDITCARD'),
			'bancontact'   => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_BANCONTACT'),
			'sofort'       => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_SOFORT'),
			'paypal'       => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_PAYPAL'),
			'paysafecard'  => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_PAYSAFECARD'),
			'banktransfer' => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_BANKTRANSFER'),
			'bitcoin'      => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_BITCOIN'),
			'kbc'          => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_KBC'),
			'belfius'      => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_BELFIUS'),
			'giftcard'     => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_GIFTCARD'),
			'inghomepay'   => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_INGHOMEPAY'),
			'eps'          => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_EPS'),
			'giropay'      => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_GIROPAY'),
			'przelewy24'   => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_PRZELEWY24'),
			'applepay'     => Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_APPLEPAY'),
		];
	}
	...
	public function getForm(Gateway $jdideal, $data): array
	{
		...
		foreach ($selected as $name)
		{
			switch ($name)
			{
				...
				case 'belfius':
					...
					break;
				case 'kbc':
					$options[] = HTMLHelper::_('select.option', 'kbc', Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_KBC'));
					break;
				case 'inghomepay':
					$options[] = HTMLHelper::_(
						'select.option',
					...
					break;
				case 'eps':
					$options[] = HTMLHelper::_('select.option', 'eps', Text::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_EPS'));
					break;
				case 'giropay':
					$options[] = HTMLHelper::_(
						'select.option',
				...
				case 'giftcard':
					...
					$cards     = [];
					$cards[]   = HTMLHelper::_('select.option', '', Text::_('COM_ROPAYMENTS_SELECT_GIFTCARD'));
					$cards[]   = HTMLHelper::_(
						'select.option',
						'nationalebioscoopbon',
						Text::_('COM_ROPAYMENTS_PAYMENT_GIFTCARD_NATIONALEBIOSCOOPBON')
					);
					$cards[]   = HTMLHelper::_(
						'select.option',
						'nationaleentertainmentcard',
						Text::_('COM_ROPAYMENTS_PAYMENT_GIFTCARD_NATIONALEENTERTAINMENTCARD')
					);
					$cards[]   = HTMLHelper::_(
						'select.option',
						'kunstencultuurcadeaukaart',
						Text::_('COM_ROPAYMENTS_PAYMENT_GIFTCARD_KUNSTENCULTUURCADEAUKAART')
					);
					$cards[]   = HTMLHelper::_(
						'select.option',
						'podiumcadeaukaart',
						Text::_('COM_ROPAYMENTS_PAYMENT_GIFTCARD_PODIUMCADEAUKAART')
					);
					$cards[]   = HTMLHelper::_(
						'select.option',
						'vvvgiftcard',
					...
					$cards[]   = HTMLHelper::_(
						'select.option',
						'webshopgiftcard',
						Text::_('COM_ROPAYMENTS_PAYMENT_GIFTCARD_WEBSHOPGIFTCARD')
					);
					$cards[]   = HTMLHelper::_(
						'select.option',
						'yourgift',
				...
				case 'ideal':
					...
					$methods = $mollie->methods->get('ideal', ['include' => 'issuers']);

					foreach ($methods->issuers as $issuer)
					{
						$banks['Nederland']['items'][] = HTMLHelper::_('select.option', $issuer->id, $issuer->name);
					}
					break;
				default:
					$options[] = '';
		...
		$jdideal->log(Text::sprintf('COM_JDIDEAL_SELECTED_CARD', $selected[0]), $data->logid);
	...
	public function sendPayment(Gateway $jdideal): void
	{
		...
		{
			throw new RuntimeException(Text::sprintf('COM_ROPAYMENTS_NO_TRANSACTION_DETAILS', 'Mollie', $logId));
		}
		...
		try
		{
			...
			if ($jdideal->get('recurring', false))
			{
				...
				$customerName  = $customer['billing']->email;
			...
			else
			{
				...
				$jdideal->log('Send customer to URL: ' . $payment->getCheckoutUrl(), $logId);
			}
	...
	public function transactionStatus(Gateway $jdideal, int $logId): array
	{
		...
		if (array_key_exists('id', $this->data) && $this->data['id'])
		{
			...
			if ($status['card'] === 'banktransfer' && !in_array($payment->status, ['paid', 'cancelled'], true))
			{
				$status['isOK']            = true;
	...
	private function createSubscription(Gateway $jdideal, int $logId): void
	{
		...
		try
		{
			...
			// Get the details
			$customerEmail = $customer['billing']->email;
			$description   = $jdideal->get('recurringDescription', 'Subscription');
			$amount        = number_format($details->amount, 2, '.', '');
