...
class plgAcymRseventspro extends acymPlugin
{
    ...
    public function replaceIndividualContent($tag)
    {
        ...
        $link = rseventsproHelper::route('index.php?option=com_rseventspro&layout=show&id='.rseventsproHelper::sef($element->id, $element->name));
        ...
        if (in_array('title', $tag->display)) $title = $element->name;
        if (in_array('short', $tag->display)) $contentText .= '<p>'.$element->small_description.'</p>';
        ...
        if (in_array('date', $tag->display)) {
            ...
            $customFields[] = [$date, acym_translation('ACYM_DATE')];
        }
        ...
        if (in_array('location', $tag->display) && !empty($element->location_id)) {
            ...
            $customFields[] = ['<a href="'.$url.'" target="_blank">'.$element->location_name.'</a>', acym_translation('COM_RSEVENTSPRO_GLOBAL_AT')];
        }
        ...
        if (in_array('cats', $tag->display)) {
            ...
            if (!empty($allcategories)) {
                foreach ($allcategories as $cat) {
                    ...
                    $url = rseventsproHelper::route('index.php?option=com_rseventspro&category='.rseventsproHelper::sef($cat->id, $cat->title));
                    $url = str_replace('administrator/', '', $url);

                    $categories[] = '<a href="'.$url.'" class="rs_cat_link" '.$style.' target="_blank">'.$cat->title.'</a>';
                }

                $customFields[] = [implode(', ', $categories), acym_translation('COM_RSEVENTSPRO_GLOBAL_CATEGORIES')];
            }
        }
        ...
        if (in_array('tags', $tag->display)) {
            ...
            if (!empty($alltags)) {
                foreach ($alltags as $oneTag) {
                    $url = rseventsproHelper::route('index.php?option=com_rseventspro&tag='.rseventsproHelper::sef($oneTag->id, $oneTag->name));
                    $url = str_replace('administrator/', '', $url);

                    $tags[] = '<a href="'.$url.'" class="rs_tag_link" target="_blank">'.$oneTag->name.'</a>';
                }
                $customFields[] = [implode(', ', $tags), acym_translation('COM_RSEVENTSPRO_GLOBAL_TAGS')];
            }
        }
