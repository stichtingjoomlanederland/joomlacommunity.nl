...
class EasyDiscussModelAttachments extends EasyDiscussAdminModel
{
	...
	/**
	 * Determind if post has attachments or not.
	 *
	 * @since	5.0
	 * @access	public
	 */
	public function hasAttachments($postId)
	{
		...
		$query = 'SELECT count(1) FROM ' . $db->qn('#__discuss_attachments');
		$query .= ' WHERE ' . $db->qn('uid') . '=' . $db->Quote($postId);
	 ...
	 * @access	public
	 */
	public function getImagesNotOptimized($limit = 10)
	{
		$db = $this->db;
		...
		$query = array(
			'SELECT a.* FROM ' . $db->qn('#__discuss_attachments') . ' AS a',
			'WHERE a.`mime` IN (' . implode(',', $images) . ')',
			'AND a.`storage`=' . $db->Quote('joomla'),
			'AND NOT EXISTS(',
			'SELECT x.`attachment_id` FROM `#__discuss_optimizer` AS x',
	...
	/**
	 * Duplicate the attachments for the duplicated post
	 *
	 * @since	5.0.0
	 * @access	public
	 */
	public function duplicate($oldPostId, $newPostId)
	{
		$attachments = $this->getPostAttachments($oldPostId);
		...
		foreach ($attachments as $attachment) {
			...
			$attachment->duplicate($newPostId);
		}
	}
}
