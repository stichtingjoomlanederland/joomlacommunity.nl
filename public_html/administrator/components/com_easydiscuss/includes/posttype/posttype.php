...
class EasyDiscussPostType
{
	...
	{
		$this->table = ED::table('Post_Types');

		// For object that is being passed in
		if (is_object($item) && !($item instanceof DiscussPost_types)) {
			$this->table->bind($item);
		}

		// If the object is DiscussConversation, just map the variable back.
		if ($item instanceof DiscussPost_types) {
			$this->table = $item;
		}
	...
	/**
	 * Retrieves the title of the post type
	 *
	 * @since	5.0.0
	 * @access	public
	 */
