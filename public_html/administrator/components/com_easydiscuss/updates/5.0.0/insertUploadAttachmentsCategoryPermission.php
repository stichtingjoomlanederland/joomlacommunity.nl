...
class EasyDiscussMaintenanceScriptInsertUploadAttachmentsCategoryPermission extends EasyDiscussMaintenanceScript
{
	public static $title = "Insert Upload Attachments Category Permission";
	public static $description = "Insert upload attachments permission for the category";
	...
	public function main()
	{
		...
		$query[] = 'WHERE ' . $db->nameQuote('action') . ' = ' . $db->Quote('uploadattachment');
		...
		if (!$aclId) {
			...
			$query[] = '(' . $db->Quote($aclId) . ', ' . $db->Quote('uploadattachment') . ', ' . $db->Quote('can upload attachments to category posts.') . ', ' . $db->Quote(1) . ', ' . $db->Quote(1) . ')';
		...
		if ($aclId && $config->get('attachment_questions')) {
			...
			if ($groups) {
				...
				$query .= " and a.`action` = " . $db->Quote('add_attachment');
				...
				foreach ($groups as $item) {
					...
					if (isset($acls[$gid])) {
						...
						// check if add_attachment is allowed in this joomla group
