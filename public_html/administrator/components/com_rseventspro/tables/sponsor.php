...
class RseventsproTableSponsor extends JTable
{
	/**
	 * @param	JDatabase	A database connector object
	 */
	public function __construct($db) {
		parent::__construct('#__rseventspro_sponsors', 'id', $db);
	}
	...
	public function uploadImage() {
		...
		$image 	= $images['image'];
		$path	= JPATH_SITE.'/components/com_rseventspro/assets/images/sponsors/';
		$width	= rseventsproHelper::getConfig('sponsor_icon_width', 'int', 250);
		...
		if ($image['size'] > 0 && $image['error'] == 0) {
			...
			if (in_array($ext,array('jpg','jpeg','png'))) {
				...
				if ($thumb->GenerateThumbnail()) {
					...
					$query->clear()
						->update($db->qn('#__rseventspro_sponsors'))
	...
	public function delete($pk = null, $children = false) {
		...
		if ($image = $db->loadResult()) {
			jimport('joomla.filesystem.file');
			if (file_exists(JPATH_SITE.'/components/com_rseventspro/assets/images/sponsors/'.$image))
				JFile::delete(JPATH_SITE.'/components/com_rseventspro/assets/images/sponsors/'.$image);
		}
