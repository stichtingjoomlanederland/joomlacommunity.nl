<?php
/**
* @package RSEvents!Pro
* @copyright (C) 2020 www.rsjoomla.com
* @license GPL, http://www.gnu.org/copyleft/gpl.html
*/
defined( '_JEXEC' ) or die( 'Restricted access' ); 
$info = rseventsproHelper::getEventInfo($this->item->id); 
$subscribers = rseventsproHelper::getSubscribers($this->item->id);
$coupons = rseventsproHelper::getCoupons($this->item->id);
$guests = rseventsproHelper::getRSVP($this->item->id); 
$cols = empty($coupons) ? 8 : 4; ?>

<div class="<?php echo RSEventsproAdapterGrid::row(); ?>">
	<div class="<?php echo RSEventsproAdapterGrid::column(4); ?>">
		<fieldset class="options-form">
			<legend><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_GENERAL_INFO'); ?></legend>

			<table class="table table-striped">
				<?php if ($info) { ?>
				<?php foreach ($info as $name => $value) { ?>
				<tr>
					<td><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_'.strtoupper($name)); ?></td>
					<td><?php echo $value; ?></td>
				</tr>
				<?php } ?>
				<?php } ?>
			</table>
		</fieldset>
	</div>
	
	<?php if ($this->item->registration) { ?>
	<div class="<?php echo RSEventsproAdapterGrid::column($cols); ?>">
		<fieldset class="options-form">
			<legend><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_TICKETS'); ?></legend>
			<table class="table table-striped">
				<thead>
					<th><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_TICKET'); ?></th>
					<th width="20%" class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_TICKET_PRICE'); ?></th>
					<th width="20%" class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_TICKET_SOLD'); ?></th>
					<th width="30%" class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_TICKET_EXPIRES'); ?></th>
				</thead>
			
				<?php if ($this->tickets) { ?>
				<?php foreach ($this->tickets as $ticket) { ?>
				<tr>
					<td><?php echo $ticket->name; ?></td>
					<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo $ticket->price > 0 ? rseventsproHelper::currency($ticket->price) : JText::_('COM_RSEVENTSPRO_GLOBAL_FREE'); ?></td>
					<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo rseventsproHelper::getTicketCount($ticket); ?></td>
					<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo $ticket->to == JFactory::getDbo()->getNullDate() ? rseventsproHelper::showdate($this->item->end) : rseventsproHelper::showdate($ticket->to); ?></td>
				</tr>
				<?php } ?>
				<?php } else { ?>
				<tr>
					<td><?php echo JText::_('COM_RSEVENTSPRO_SUBSCRIPTION_FREE_ENTRANCE'); ?></td>
					<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>">-</td>
					<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo rseventsproHelper::getTicketCountNoEntrance($this->item->id); ?></td>
					<td><?php echo rseventsproHelper::showdate($this->item->end); ?></td>
				</tr>
				<?php } ?>
			</table>
		</fieldset>
	</div>
	<?php } ?>
	
	<?php if ($coupons) { ?>
	<div class="<?php echo RSEventsproAdapterGrid::column(3); ?>">
		<fieldset class="options-form">
			<legend><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_COUPONS'); ?></legend>
			<table class="table table-striped">
				<thead>
					<th><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_COUPON'); ?></th>
					<th class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_COUPON_USED'); ?></th>
				</thead>
				<tbody>
					<?php foreach ($coupons as $coupon) { ?>
					<tr>
						<td><?php echo $coupon->coupon; ?></td>
						<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo $coupon->nr; ?></td>
					</tr>
					<?php } ?>
				</tbody>
			</table>
		</fieldset>
	</div>
	<?php } ?>
</div>

<div class="<?php echo RSEventsproAdapterGrid::row(); ?>">
	<?php if ($this->item->registration) { ?>
	<fieldset class="options-form">
		<legend><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_SUBSCRIBERS'); ?></legend>
		<?php if ($subscribers) { ?>
		<table class="table table-striped">
			<thead>
				<th><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_SUBSCRIBER_NAME'); ?></th>
				<th class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_SUBSCRIBER_DATE'); ?></th>
				<th class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_SUBSCRIBER_GATEWAY'); ?></th>
				<th class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_SUBSCRIBER_STATUS'); ?></th>
			</thead>
			<tbody>
				<?php foreach ($subscribers as $subscriber) { ?>
				<tr>
					<td>
						<a target="_blank" href="<?php echo JRoute::_('index.php?option=com_rseventspro&task=subscription.edit&id='.$subscriber->id); ?>"><?php echo $subscriber->name; ?></a> (<?php echo $subscriber->email; ?>)
					</td>
					<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo rseventsproHelper::showdate($subscriber->date,null,true); ?></td>
					<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>">
						<?php $payment = rseventsproHelper::getPayment($subscriber->gateway); ?>
						<?php echo $payment ? $payment : '-'; ?>
					</td>
					<td class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>"><?php echo rseventsproHelper::getStatuses($subscriber->state); ?></td>
				</tr>
				<?php } ?>
			</tbody>
			<tfoot>
				<tr>
					<td colspan="4" class="<?php echo RSEventsproAdapterGrid::styles(array('center')); ?>">
						<a href="<?php echo JRoute::_('index.php?option=com_rseventspro&view=subscriptions&filter_event='.$this->item->id, false); ?>" class="btn btn-primary"><?php echo JText::_('COM_RSEVENTSPRO_VIEW_ALL'); ?></a>
					</td>
				</tr>
			</tfoot>
		</table>
		<?php } else { ?>
		<?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_NO_SUBSCRIBERS'); ?>
		<?php } ?>
	</fieldset>
	<?php } ?>
	
	<?php if ($this->item->rsvp) { ?>
	<fieldset class="options-form">
		<legend><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_GUESTS'); ?></legend>
		<?php if ($guests) { ?>
		<table class="table table-striped">
			<thead>
				<th><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_GUEST_NAME'); ?></th>
				<th class="center"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_GUEST_DATE'); ?></th>
				<th class="center"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_GUEST_STATUS'); ?></th>
			</thead>
			<tbody>
				<?php foreach ($guests as $guest) { ?>
				<tr>
					<td><?php echo $guest->name; ?> (<?php echo $guest->email; ?>)</td>
					<td class="center"><?php echo rseventsproHelper::showdate($guest->date,null,true); ?></td>
					<td class="center"><?php echo rseventsproHelper::RSVPStatus($guest->rsvp); ?></td>
				</tr>
				<?php } ?>
			</tbody>
			<tfoot>
				<tr>
					<td colspan="4" class="center">
						<a href="<?php echo JRoute::_('index.php?option=com_rseventspro&view=rsvp&id='.$this->item->id, false); ?>" class="btn btn-primary"><?php echo JText::_('COM_RSEVENTSPRO_VIEW_ALL'); ?></a>
					</td>
				</tr>
			</tfoot>
		</table>
		<?php } else { ?>
		<?php echo JText::_('COM_RSEVENTSPRO_EVENT_DASH_NO_GUESTS'); ?>
		<?php } ?>
	</fieldset>
	<?php } ?>
</div>