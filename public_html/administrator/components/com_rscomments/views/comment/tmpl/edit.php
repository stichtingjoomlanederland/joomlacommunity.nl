...
<?php
	...
	foreach ($this->form->getFieldset() as $field) {
		...
		if($field->name == 'jform[ip]') {
			...
			echo JHtml::_('rsfieldset.element', '<label id="jform_date-lbl" class="'.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('COM_RSCOMMENTS_COMMENT_DATE_DESC')).'" for="jform_ip">'.JText::_('COM_RSCOMMENTS_COMMENT_DATE').'</label>', '<button class="btn btn-info btn-small fltlft" type="button"><i class="icon-clock"></i> '.RSCommentsHelperAdmin::showDate($this->item->date).'</button>');
			...
			echo JHtml::_('rsfieldset.element', '<label id="jform_ip-lbl" class="'.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('COM_RSCOMMENTS_COMMENT_COMPONENT_DESC')).'" for="jform_ip">'.JText::_('COM_RSCOMMENTS_COMMENT_COMPONENT').'</label>', '<span class="btn btn-info btn-small fltlft" style="margin-top:5px;">'.RSCommentsHelperAdmin::component($this->item->option).' &mdash; ID: '.$this->item->id.'</span>');
