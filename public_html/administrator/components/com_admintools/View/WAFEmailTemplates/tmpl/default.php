...
<form action="index.php" method="post" name="adminForm" id="adminForm" class="akeeba-form">
		...
		<div class="akeeba-filter-bar akeeba-filter-bar--left akeeba-form-section akeeba-form--inline">
			<div class="akeeba-filter-element akeeba-form-group">
				<input type="text" name="reason"
					   placeholder="<?php echo Text::_('COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_REASON_SELECT'); ?>"
					   id="filter_reason" onchange="document.adminForm.submit();"
					   value="<?php echo $this->escape($this->filters['reason']); ?>"
					   title="<?php echo Text::_('COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_REASON_SELECT'); ?>" />
			</div>

			<div class="akeeba-filter-element akeeba-form-group">
				<input type="text" name="subject"
					   placeholder="<?php echo Text::_('COM_ADMINTOOLS_WAFEMAILTEMPLATES_FIELD_SUBJECT_LBL'); ?>"
					   id="filter_subject" onchange="document.adminForm.submit();"
					   value="<?php echo $this->escape($this->filters['subject']); ?>"
					   title="<?php echo Text::_('COM_ADMINTOOLS_WAFEMAILTEMPLATES_FIELD_SUBJECT_LBL'); ?>" />
			</div>
	...
	<table class="akeeba-table akeeba-table--striped" id="itemsList">
		...
		<tr>
			...
			<th>
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_WAFEMAILTEMPLATES_FIELD_SUBJECT_LBL', 'subject', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th style="width:8%">
				<?php echo HTMLHelper::_('grid.sort', 'JPUBLISHED', 'enabled', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th style="width: 20%">
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_COMMON_LANGUAGE', 'language', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
		...
		if ($this->items):
			...
			foreach ($this->items as $row):
				...
				?>
				<tr>
					<td><?php echo HTMLHelper::_('grid.id', ++$i, $row->admintools_waftemplate_id); ?></td>
					<td>
