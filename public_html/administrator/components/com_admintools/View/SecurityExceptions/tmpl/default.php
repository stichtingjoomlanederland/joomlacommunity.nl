...
<form action="index.php" method="post" name="adminForm" id="adminForm" class="akeeba-form">
	<section class="akeeba-panel--33-66 akeeba-filter-bar-container">
		<div class="akeeba-filter-bar akeeba-filter-bar--left akeeba-form-section akeeba-form--inline">
			<div class="akeeba-filter-element akeeba-form-group akeeba-filter-joomlacalendarfix">
				<?php echo HTMLHelper::_('calendar', $this->filters['from'], 'datefrom', 'datefrom', '%Y-%m-%d', ['class' => 'input-small']); ?>
			</div>

			<div class="akeeba-filter-element akeeba-form-group akeeba-filter-joomlacalendarfix">
				<?php echo HTMLHelper::_('calendar', $this->filters['to'], 'dateto', 'dateto', '%Y-%m-%d', ['class' => 'input-small']); ?>
			</div>

			<div class="akeeba-filter-element akeeba-form-group">
				<input type="text" name="ip"
					   placeholder="<?php echo Text::_('COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_IP'); ?>"
					   id="filter_ip" onchange="document.adminForm.submit();"
					   value="<?php echo $this->escape($this->filters['ip']); ?>"
					   title="<?php echo Text::_('COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_IP'); ?>" />
			</div>
	...
	<table class="akeeba-table akeeba-table--striped" id="itemsList">
		...
		<tr>
			...
			<th style="width:17%">
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_LOGDATE', 'logdate', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th style="width:15%">
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_IP', 'ip', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th style="width: 15%">
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_REASON', 'reason', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th>
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_URL', 'url', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
		</tr>
		</thead>
		<tfoot>
		...
		if ($this->items):
			...
			foreach ($this->items as $row):
				...
				if ($row->extradata)
				{
					...
					[$moreinfo, $techurl] = explode('|', $row->extradata);

					$reason .= '&nbsp;' . HTMLHelper::_('tooltip', strip_tags(htmlspecialchars($moreinfo, ENT_COMPAT, 'UTF-8')), '', 'tooltip.png', '', $techurl);
				}
				...
				if ($row->block)
				{
					...
					$ip .= 'title="' . Text::_('COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_UNBAN') . '">';
				...
				else
				{
					...
					$ip .= 'title="' . Text::_('COM_ADMINTOOLS_LBL_SECURITYEXCEPTION_BAN') . '">';
