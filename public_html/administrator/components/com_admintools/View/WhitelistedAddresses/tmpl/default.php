...
<form action="index.php" method="post" name="adminForm" id="adminForm" class="akeeba-form">
	...
	<section class="akeeba-panel--33-66 akeeba-filter-bar-container">
		<div class="akeeba-filter-bar akeeba-filter-bar--left akeeba-form-section akeeba-form--inline">
			<div class="akeeba-filter-element akeeba-form-group">
				<input type="text" name="ip" placeholder="<?php echo \JText::_('COM_ADMINTOOLS_LBL_WHITELISTEDADDRESS_IP'); ?>"
					   id="filter_ip" onchange="document.adminForm.submit();"
					   value="<?php echo $this->escape($this->filters['ip']); ?>"
					   title="<?php echo \JText::_('COM_ADMINTOOLS_LBL_WHITELISTEDADDRESS_IP'); ?>"/>
			</div>

			<div class="akeeba-filter-element akeeba-form-group">
				<input type="text" name="description" placeholder="<?php echo \JText::_('COM_ADMINTOOLS_LBL_WHITELISTEDADDRESS_DESCRIPTION'); ?>"
					   id="filter_description" onchange="document.adminForm.submit();"
					   value="<?php echo $this->escape($this->filters['description']); ?>"
					   title="<?php echo \JText::_('COM_ADMINTOOLS_LBL_WHITELISTEDADDRESS_DESCRIPTION'); ?>"/>
			</div>
		</div>
	...
	<table class="akeeba-table akeeba-table--striped" id="itemsList">
		...
		<tr>
			...
			<th>
				<?php echo \JHtml::_('grid.sort', 'COM_ADMINTOOLS_LBL_WHITELISTEDADDRESS_IP', 'ip', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th>
				<?php echo \JHtml::_('grid.sort', 'COM_ADMINTOOLS_LBL_WHITELISTEDADDRESS_DESCRIPTION', 'description', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
		</tr>
		</thead>
		<tfoot>
		...
		<?php if (!count($this->items)):?>
			<tr>
				<td colspan="6">
					<?php echo JText::_('COM_ADMINTOOLS_ERR_WHITELISTEDADDRESS_NOITEMS')?>
				</td>
			</tr>
		<?php endif;?>
		<?php
		if ($this->items):
			...
			foreach($this->items as $row):
				$edit = 'index.php?option=com_admintools&view=WhitelistedAddresses&task=edit&id='.$row->id;
				?>
				<tr>
	...
	<div class="akeeba-hidden-fields-container">
		...
		<input type="hidden" name="view" id="view" value="WhitelistedAddresses"/>
