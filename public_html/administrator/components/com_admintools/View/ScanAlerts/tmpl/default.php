...
$returnUrl = Route::_('index.php?option=com_admintools&view=ScanAlerts&id=' . $scan_id);

?>
<div class="akeeba-container--50-50">
	<div></div>
	<div>
		<a href="index.php?option=com_admintools&view=Scans&task=edit&id=<?php echo $scan_id ?>&returnurl=<?php echo base64_encode($returnUrl) ?>"
		   class="akeeba-btn--green--small" style="float:right">
			<span class="icon-pencil"></span>
			<?php echo Text::_('COM_ADMINTOOLS_SCANALERTS_EDIT_COMMENT') ?>
		</a>
		<span id="showComment" class="akeeba-btn--primary--small" style="margin-right: 10px;float:right">
            ...
            <?php echo Text::_('COM_ADMINTOOLS_SCANALERTS_SHOWCOMMENT') ?>
        </span>
...
<form action="index.php" method="post" name="adminForm" id="adminForm" class="akeeba-form">
		...
		<div class="akeeba-filter-bar akeeba-filter-bar--left akeeba-form-section akeeba-form--inline">
			...
			<div class="akeeba-filter-element akeeba-form-group">
				<input type="text" name="path"
					   placeholder="<?php echo Text::_('COM_ADMINTOOLS_LBL_SCANALERTS_PATH'); ?>"
					   id="filter_path" onchange="document.adminForm.submit();"
					   value="<?php echo $this->escape($this->filters['path']); ?>"
					   title="<?php echo Text::_('COM_ADMINTOOLS_LBL_SCANALERTS_PATH'); ?>" />
			</div>
		</div>
	...
	<table class="akeeba-table akeeba-table--striped" id="itemsList">
		...
		<tr>
			...
			<th>
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_LBL_SCANALERTS_PATH', 'path', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th style="width: 100px">
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_LBL_SCANALERTS_STATUS', 'filestatus', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th style="width: 100px;">
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_LBL_SCANALERTS_THREAT_SCORE', 'threat_score', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th style="width: 100px;">
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_LBL_SCANALERTS_ACKNOWLEDGED', 'acknowledged', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
		...
		if ($this->items):
			...
			foreach ($this->items as $row):
				...
				$html_status = '<span class="admintools-scanfile-' . $fstatus . $extra_class . '">' . Text::_('COM_ADMINTOOLS_LBL_SCANALERTS_STATUS_' . $fstatus) . '</span>';
				...
				?>
				<tr>
					<td><?php echo HTMLHelper::_('grid.id', ++$i, $row->admintools_scanalert_id); ?></td>
					<td>
						<?php echo $html_path; ?>
					</td>
					<td>
