		...
		<div class="rscomments-accordion">
			...
			<div class="rscomments-accordion-content<?php echo $this->config->show_form ? ' open' : ''; ?>" id="rscomments-accordion-content"<?php echo !$this->config->show_form ? ' style="display: none;"' : ''; ?>>
				...
				<div class="row">
					<div class="form-group col-md-6">
						...
						<?php } ?>
						<div class="controls">
							<input <?php echo $this->disable; ?> type="text" class="col-md-11 required" id="rsc_name" name="jform[name]" value="<?php echo $this->user->get('name'); ?>" <?php if (!$this->config->show_labels)
							{ ?>placeholder="<?php echo JText::_('COM_RSCOMMENTS_COMMENT_NAME') . $required; ?>"<?php } ?> />
						</div>
					</div>
					<div class="form-group col-md-6">
						<?php if ($this->config->show_labels)
						...
						<?php } ?>
						<div class="controls">
							<input <?php echo $this->disable; ?> type="text" class="col-md-11 required" id="rsc_email" name="jform[email]" value="<?php echo $this->user->get('email'); ?>" <?php if (!$this->config->show_labels)
							{ ?>placeholder="<?php echo JText::_('COM_RSCOMMENTS_COMMENT_EMAIL') . $required; ?>"<?php } ?> />
						</div>
					</div>
				</div>
					...
					<div class="row">
						...
						{ ?>
							<div class="form-group col-md-6">
								...
								<?php } ?>
								<div class="controls">
									<input type="text" class="col-md-11" id="rsc_subject" name="jform[subject]" value="" <?php if (!$this->config->show_labels)
									{ ?>placeholder="<?php echo JText::_('COM_RSCOMMENTS_COMMENT_SUBJECT'); ?>"<?php } ?> />
								</div>
							</div>
						<?php } ?>
						...
						{ ?>
							<div class="form-group col-md-6">
								...
								<?php } ?>
								<div class="controls">
									<input type="text" class="col-md-11" id="rsc_website" name="jform[website]" value="" <?php if (!$this->config->show_labels)
									{ ?>placeholder="<?php echo JText::_('COM_RSCOMMENTS_COMMENT_WEBSITE'); ?>"<?php } ?> />
								</div>
							</div>
						<?php } ?>
					</div>
				...
				{ ?>
					<div class="row">
						<div class="form-group col-md-6">
							...
							<?php } ?>
							<div class="controls">
								<input type="text" class="col-md-10" id="rsc_location" name="jform[location]" value="" <?php if (!$this->config->show_labels)
								{ ?>placeholder="<?php echo JText::_('COM_RSCOMMENTS_COMMENT_LOCATION'); ?>"<?php } ?> />
							</div>
						</div>
						<div class="form-group col-md-6">
							<label class="control-label" for="rsc_location">

							</label>
							<div class="controls">
								<a id="rsc_detect_btn" href="javascript:void(0)" class="btn btn-primary btn-xs"><?php echo JText::_('COM_RSCOMMENTS_COMMENT_DETECT_LOCATION'); ?></a>
							</div>
						</div>
					</div>
				<?php } ?>
				...
				<div class="row">
					<div class="form-group col-md-12" id="rscomments-comment-area">
						<div>
							<textarea id="rsc_comment" class="form-control required" name="jform[comment]" rows="5" onkeydown="rsc_comment_cnt('rsc_comment','commentlen','<?php echo $clength; ?>');" onkeyup="rsc_comment_cnt('rsc_comment','commentlen','<?php echo $clength; ?>');" maxlength="<?php echo $clength; ?>" placeholder="<?php echo JText::_('COM_RSCOMMENTS_COMMENT_COMMENT'); ?>"></textarea>
						</div>
					</div>
					<div class="col-md-12">
						...
						{ ?>
							<div class="form-group pull-left">
								<p class="char-left muted">
									<span id="commentlen"><?php echo $clength; ?></span> <?php echo JText::_('COM_RSCOMMENTS_CHARS_LEFT'); ?>
								</p>
							</div>
						<?php } ?>
					</div>
					...
					<div class="row">
						...
						{ ?>
							<div class="form-group col-md-3">
								<div class="checkbox">
									<label class="<?php echo RSTooltip::tooltipClass(); ?>" title="<?php echo RSTooltip::tooltipText(JText::_('COM_RSCOMMENTS_SUBSCRIBE_DESC')); ?>">
										<input type="checkbox" name="jform[subscribe_thread]" value="1"/>
										<?php echo JText::_('COM_RSCOMMENTS_SUBSCRIBE'); ?>
									</label>
								</div>
							</div>
						<?php } ?>
						...
						{ ?>
							<div class="form-group col-md-9">
								<div class="checkbox">
									<label>
										<input type="checkbox" name="jform[rsc_terms]" value="1"/>
						...
						<div class="col-md-12">
							...
							<a id="rscomments-refresh-captcha" style="border-style: none" href="javascript:void(0)" onclick="rsc_refresh_captcha('<?php echo $this->root; ?>','<?php echo RSCommentsHelper::route('index.php?option=com_rscomments&task=captcha&type=form'); ?>');">
								<img src="<?php echo RSCommentsHelper::ImagePath('refresh.png'); ?>" alt="<?php echo JText::_('COM_RSCOMMENTS_REFRESH_CAPTCHA'); ?>" onclick="this.blur()"/>
										...
										<input type="text" name="jform[captcha]" id="submit_captcha" size="40" value="" class="col-md-5 <?php echo RSTooltip::tooltipClass(); ?> required" title="<?php echo RSTooltip::tooltipText($this->config->captcha_cases ? JText::_('COM_RSCOMMENTS_CAPTCHA_CASE_SENSITIVE') : JText::_('COM_RSCOMMENTS_CAPTCHA_CASE_INSENSITIVE')); ?>"/>
									<?php }
									else if ($this->config->captcha == 1)
									{
			...
			<span id="rsc_loading_form" class="rsloading" style="display:none;">
				<img src="<?php echo RSCommentsHelper::ImagePath('loader.gif'); ?>" alt=""/>
						...
						<button type="button" onclick="rsc_reset_form();" id="rsc_reset" class="btn btn-reset"><?php echo JText::_('COM_RSCOMMENTS_RESET'); ?></button>
