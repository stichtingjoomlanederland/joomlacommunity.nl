...
<div class="well">
	...
	{ ?>
		<div class="rs_subscribe">
			...
			<?php if (!empty($this->tickets) && !$this->thankyou)
			{ ?>
				<script type="text/javascript"><?php if ($this->event->ticketsconfig) { ?>rsepro_update_total();
					<?php } else { ?>rs_get_ticket(jQuery('#RSEProTickets').val());<?php } ?></script><?php } ?>
		</div>
	<?php } else
	{ ?>
	<form action="<?php echo rseventsproHelper::route('index.php?option=com_rseventspro&layout=subscribe'); ?>" method="post" name="adminForm" id="adminForm" class="form-horizontal">
		<div class="rs_subscribe">
				...
				<div class="form-group">
					...
					<div class="controls">
						<a href="javascript:void(0);" onclick="RSopenModal();">
							...
							<span id="rsepro_cart"><?php echo JText::_('COM_RSEVENTSPRO_SELECT_TICKETS'); ?></span>
						</a>
					</div>
				</div>
			...
			{ ?>
				...
				<div class="form-group">
					...
					<div class="controls">
						...
						<select name="number" id="number" class="input-mini" onchange="<?php echo $this->js; ?>">
			...
			{ ?>
				...
				<div class="form-group" id="rsepro_payment">
