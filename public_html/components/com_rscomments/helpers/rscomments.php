...
//				}
	...
	public static function showRSComments($option, $id, $template = null, $container = null, $override = false) {
		...
		$return  = '<div class="rscomments" id="rscomments-'.$hash.'" data-rsc-id="'.$id.'" data-rsc-option="'.$option.'">';
	...
	public static function loadScripts() {
		...
		if (!$loaded) {
			...
			if (RSCommentsHelper::isJ4()) {
				JHtml::script('com_rscomments/modal.js', array('relative' => true, 'version' => 'auto'));
			}
	...
	public static function loadRecaptcha($hash) {
		...
		if ($config->captcha == 2 && isset($permissions['captcha']) && $permissions['captcha']) {
			...
			if ($config->recaptcha_new_size == 'invisible') {
				$script[] = "\tdocument.getElementById('rscomments-".$hash."').onclick = function() {";
				...
				$script[] = "\t\tdocument.getElementById('rscomments-".$hash."').onclick = null;";
				$script[] = "\t}";
			} else {
				$script[] = "\tvar id = grecaptcha.render('rsc-g-recaptcha-".$hash."', {";
				$script[] = "\t\t'sitekey': '".htmlentities($config->recaptcha_new_site_key, ENT_QUOTES, 'UTF-8')."',";
	...
	public static function sendEmailSubscriptions($row) {
		$db			 = JFactory::getDbo();
		$query		 = $db->getQuery(true);
		...
		$query->clear()
			->select($db->qn('emails'))
			->from($db->qn('#__rscomments_comments'))
			->where($db->qn('IdComment').' = '.$db->q($row->IdComment));
		$db->setQuery($query);
		$emailsSent = $db->loadResult();
	...
	public static function renderModal($selector, $options = [], $body = '') {
		try {
			return JHtml::_('bootstrap.renderModal', $selector, $options, $body);
		} catch(Exception $e) {
			$layoutData = [
				'selector' => $selector,
				'params'   => $options,
			...
			return JLayoutHelper::render('libraries.html.bootstrap.modal.main', $layoutData);
		}
	}
}
