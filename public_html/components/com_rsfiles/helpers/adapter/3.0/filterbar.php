...
class RSFilterBar {
	...
	public function show() {
		if ($this->sortFields || $this->orderDir) {
			JFactory::getDocument()->addScript(JURI::root(true).'/administrator/components/com_rsfiles/assets/js/ordertable.js');
		}
		?>
		<div id="filter-bar" class="btn-toolbar">
			...
			<?php if ($this->root) { ?>
			<div class="btn-group pull-right">
				<label for="root" class="element-invisible"><?php echo JText::_('ROOT');?></label>
				<select name="root_files" id="root" class="input-medium" onchange="change_root(this.value);">
					<?php echo JHtml::_('select.options', $this->root, 'value', 'text', rsfilesHelper::getRoot(), false); ?>
				</select>
			</div>
			<?php } ?>
	...
	public function orderingBody($order, $default, $pagination, $i, $total, $task) {
		...
		if ($this->listOrder == $default) {
			$saveOrderingUrl = 'index.php?option=com_rsfiles&task='.$task.'.saveOrderAjax&tmpl=component';
