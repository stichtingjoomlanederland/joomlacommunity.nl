...
class rseventsproHelper
{
	...
	public static function loadScripts() {
		...
		$vars[] = "var rsepro_root = '".addslashes(JURI::root(true).'/'.($app->isClient('administrator') ? 'administrator/' : ''))."';";
		$vars[] = "var rsepro_modal_width = '".rseventsproHelper::getConfig('modal_width','int', 800)."';";
		$vars[] = "var rsepro_modal_height = '".rseventsproHelper::getConfig('modal_height','int', 600)."';";
		$vars[] = "var rsepro_muted_class = '".RSEventsproAdapterGrid::styles(array('muted'))."'";
		
		$doc->addScriptDeclaration(implode("\n", $vars));
		
		if (rseventsproHelper::isJ4()) {
			JHtml::script('com_rseventspro/modal.js', array('relative' => true, 'version' => 'auto'));
		}
		...
		if ($app->isClient('administrator')) {
			if (!rseventsproHelper::isJ4()) {
				...
				JHtml::script('com_rseventspro/validation.js', array('relative' => true, 'version' => 'auto'));
			}
			...
			// Add JS files
			if ($doc->getType() == 'html') {
				$doc->addCustomTag('<script src="'.JHtml::script('com_rseventspro/site.js', array('relative' => true, 'pathOnly' => true, 'version' => 'auto')).'?'.$version.'" type="text/javascript"></script>');
			}
		...
		if (rseventsproHelper::isJ4()) {
			...
			JHtml::stylesheet('com_rseventspro/style40.css', array('relative' => true, 'version' => 'auto'));
		}
	}
	...
	public static function loadjQuery($noconflict = true) {
		...
		// Set this to always on since Joomla! 4 does not include the jQuery library
		if (rseventsproHelper::isJ4() && JFactory::getApplication()->isClient('site')) {
			$enabled = true;
		}
	...
	public static function loadBootstrap($force = false) {
		...
		if ($document->getType() == 'html') {
			$document->addCustomTag('<script src="'.JHtml::script('com_rseventspro/bootstrap.fix.js', array('relative' => true, 'pathOnly' => true, 'version' => 'auto')).'?'.$version.'" type="text/javascript"></script>');
		}
	}
	...
	public static function loadModal() {
		if (rseventsproHelper::getConfig('modaltype','int',1) == 2) {
			if (JFactory::getDocument()->getType() == 'html') {
				...
				JFactory::getDocument()->addCustomTag('<script src="'.JHtml::script('com_rseventspro/jquery.colorbox.min.js', array('relative' => true, 'pathOnly' => true, 'version' => 'auto')).'?'.$version.'" type="text/javascript"></script>');
			}
	...
	public static function archive() {
		...
		if (!empty($container)) {
			...
			JFactory::getApplication()->triggerEvent('onrsepro_beforeArchive',array(array('events'=>&$container)));
	...
	public static function subMenu() {
		...
		$views  = array('events','locations','categories','tags','speakers','sponsors','subscriptions','discounts','payments','groups','users','imports','backup','messages','settings');
		
		$app->triggerEvent('onrsepro_adminSubMenu',array(array('views' => &$views)));
	...
	public static function pdf($version = false) {
		$db		= JFactory::getDBO();
		$query	= $db->getQuery(true);
		...
		if ($enabled && file_exists(JPATH_SITE.'/plugins/system/rsepropdf/rsepropdf.php')) {
			if ($version) {
				if (file_exists(JPATH_SITE.'/plugins/system/rsepropdf/rsepropdf.xml')) {
					$content = file_get_contents(JPATH_SITE.'/plugins/system/rsepropdf/rsepropdf.xml');
					preg_match('#<version>(.*?)<\/version>#is', $content, $match);
	...
	public static function getPayments($addNone = false, $available = null) {
		...
		//payment plugins
		$payment_methods = $app->triggerEvent('onrsepro_addOptions');
	...
	public static function modalClose($script = true, $parent = false) {
		...
		if ($modal == 2) {
			...
			$html .= 'jQuery(\'.modal .close, .modal .btn-close\').click();';
		}
	...
	public static function getPayment($pid) {
		...
		if (!isset($payments[$pid])) {
			...
			$db->setQuery($query);
			$payments[$pid] = $db->loadResult();
		}
		
		
       
        if (!empty($payments[$pid])) {
            return $payments[$pid];
		}
       
        $gateway = $app->triggerEvent('onrsepro_name',array(array('gateway' => $pid)));
	...
	public static function confirm($ids, $registration = false, $update = true) {
		...
		JFactory::getApplication()->triggerEvent('onrsepro_afterSubscriberConfirm',array(array('ids'=>$ids)));
		
		// Get subscription details
		$query->clear()
			->select($db->qn('ide'))->select($db->qn('name'))->select($db->qn('email'))->select($db->qn('idu'))->select($db->qn('lang'))
			->select($db->qn('date'))->select($db->qn('discount'))->select($db->qn('early_fee'))->select($db->qn('late_fee'))
		...
		JFactory::getApplication()->triggerEvent('onrsepro_cartEmails', array(array('ids' => $ids, 'override' => &$override, 'registration' => $registration)));
	...
	public static function denied($ids) {
		...
		JFactory::getApplication()->triggerEvent('onrsepro_cartEmails', array(array('ids' => $ids, 'override' => &$override, 'denied' => true)));
	...
	public static function exportSubscribersCSV($query) {
		...
		JFactory::getApplication()->triggerEvent('onrsepro_exportEventSubscribers', array(array('query' => $query)));
		...
		if (!empty($subscribers)) {
			...
			if (rseventsproHelper::getConfig('export_headers')) {
				...
				$csv .= '"'.rseventsproHelper::fixValue(JText::_('COM_RSEVENTSPRO_SUBSCRIBER_EVENT_NAME')).'",';
				...
				if (file_exists(JPATH_SITE.'/components/com_rsform/rsform.php')) {
					if ($single) {
						...
						$csv .= ',"'.rseventsproHelper::fixValue(JText::_('COM_RSEVENTSPRO_SUBSCRIBER_EXPORT_HEADER_RSFORM')).'"';
					}					
				}
			...
			foreach ($subscribers as $subscriber) {
				...
				foreach ($tickets as $ticket) { 
					...
					$csv .= '"'.$db->escape(rseventsproHelper::fixValue($subscriber->event)).'",';
					...
					if (file_exists(JPATH_SITE.'/components/com_rsform/rsform.php')) {
						if ($subscriber->SubmissionId) {
							if ($submission = rseventsproHelper::getSubmission($subscriber->SubmissionId, true, $single)) {
								if ($single) {
									$csv .= ','.'"'.implode('","',$submission).'"';
								} else {
									$csv .= ','.'"'.implode(';',$submission).'"';
								}
							}
		...
		$file = 'Subscribers '.JFactory::getDate()->format('Y-m-d').'.csv';
	...
	public static function getSubmission($id, $escape = true, $single = false) {
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
		...
		if (!empty($results)) {
			...
			foreach ($headers as $i => $header) {
				if (isset($values[$header])) {
					...
					$data[] = $single ? rseventsproHelper::fixValue($values[$header]) : $header.'='.rseventsproHelper::fixValue($values[$header]);
				} else $data[] = $single ? '' : $header.'=';
			}
		}
	...
	public static function copy($eventID, $dates) {
		...
		if (!empty($dates)) {
			...
			$query->clear()
				...
				->where($db->qn('type').' IN ('.$db->q('category').','.$db->q('tag').','.$db->q('groups').','.$db->q('speaker').')');
			...
			JFactory::getApplication()->triggerEvent('onrsepro_copyEventVariables', array(array('vars' => &$vars, 'id' => $parent)));
				...
				if (!empty($tickets)) {
					foreach ($tickets as $ticket) {
						...
						if (!$ticketid) {
							...
							JFactory::getApplication()->triggerEvent('onrsepro_copyEventFields', array(array('vars' => $vars, 'old' => $ticket->id, 'new' => $db->insertid())));
						}
					}
				}
	...
	public static function remove($id) {
		...
		$app->triggerEvent('onrsepro_beforeEventDeleteTickets', array(array('id' => $id)));
		...
		$app->triggerEvent('onrsepro_deleteCartSubscriptions', array(array('id' => $id)));
	...
	public static function details($id, $itemid = null, $content = false) {
		...
		if (!isset($events[$hash])) {
			...
			$db->setQuery($query);
			$events[$hash] = $db->loadObjectList();
		}
		
		$ids = array();
		if ($events[$hash]) {
			foreach ($events[$hash] as $event) {
				$ids[] = $event->id;
				$ids[] = $event->parent;
			}
		}
		...
		foreach ($events[$hash] as $event) {
			...
			if ($event->options) {
				try {
					...
					if (is_array($event->options)) {
						$registry->loadArray($event->options);
					} else {
			...
			// Sponsors
			$container['sponsors'] = rseventsproHelper::getSponsors($event->id);
			...
			if (!empty($eventtickets[$event->id])) {
				foreach ($eventtickets[$event->id] as $ticket) {
					...
					$purchased		= self::getPurchasedTickets($ticket->id);
			...
			if (!empty($repeats[$idr])) {
				$now = JFactory::getDate();
				foreach ($repeats[$idr] as $i => $childevent) {
					if (isset($event->options['show_active_child_events']) && $event->options['show_active_child_events']) {
						$date = $childevent->allday ? $childevent->start : $childevent->end;
			...
			$app->triggerEvent('onrsepro_details',array(array('details'=>&$container)));
	...
	protected static function getEventRepeats($ids) {
		...
		if (!isset($repeats[$hash])) {
			...
			$db->setQuery($query);
			$repeats[$hash] = $db->loadObjectList();
		}
		
		$return = array();
		foreach ($repeats[$hash] as $event) {
	...
	public static function getUser($uid, $type = 'owner', $name = null) {
		...
		if (!isset($username[$hash])) {
			...
			if ($option == 0) {
				$username[$hash] = is_null($name) ? $user->name : $name;
			} elseif ($option == 1) {
				...
				if (file_exists(JPATH_SITE.'/components/com_community/libraries/core.php')) {
					...
					$username[$hash] = $user->getDisplayName();
				} else $username[$hash] = $user->name;
			} elseif ($option == 3) {
				$query->clear()
				...
				if ($details->firstname && $details->lastname) {
					$username[$hash] = $details->firstname.' '.$details->middlename.' '.$details->lastname;
				} else {
				...
				$username[$hash] = !empty($name) ? $name : $user->name;
			} else {
				$username[$hash] = $user->name;
			}
		}
	...
	public static function getEventFiles($id) {
		...
		if (!empty($files[$id])) {
			...
			if (!isset($dates[$id])) {
				...
				$db->setQuery($query);
				$dates[$id] = $db->loadObject();
			}
			
			$now	= new DateTime('now', new DateTimezone('UTC'));
			$start	= new DateTime($dates[$id]->start, new DateTimezone('UTC'));
			$end	= new DateTime($dates[$id]->end, new DateTimezone('UTC'));
	...
	public static function registered($id) {
		...
		if (!isset($registered[$id])) {
			...
			$db->setQuery($query);
			$registered[$id] = $db->loadResult() ? true : false;
		}
			...
			switch ($avatar) {
				...
				case 'easydiscuss':
					...
					if (file_exists($file)) {
						...
						if (file_exists(JPATH_ADMINISTRATOR.'/components/com_easydiscuss/includes/gettable/gettable.php')) {
	...
	public static function getProfile($type, $id) {
		...
		$url	 = '';
		$hash	 = md5($type.$profile.$id);
		
		if ($id == 0) {
			return $url;
		}
		
		static $profileURL = array();
		
		if (!isset($profileURL[$hash])) {
			if (!empty($profile)) {
				// JomSocial
				if ($profile == 1) {
					if (file_exists(JPATH_SITE.'/components/com_community/libraries/core.php')) {
						include_once(JPATH_SITE.'/components/com_community/libraries/core.php');
						$profileURL[$hash] = CRoute::_('index.php?option=com_community&view=profile&userid='.$id);
					}
				}
				// Community Builder
				else if ($profile == 2) {
					if (file_exists(JPATH_ADMINISTRATOR.'/components/com_comprofiler/plugin.foundation.php')) {
						...
						$profileURL[$hash] = $_CB_framework->userProfileUrl( $id, true );
					}
				} 
				// EasySocial
				else if ($profile == 3) {
					...
					if (file_exists($file)) {
						...
						$profileURL[$hash] = Foundry::user($id)->getPermalink();
					}
				}
				// EasyDiscuss
				else if ($profile == 4) {
					...
					if (file_exists($file)) {
						...
						$profileURL[$hash] = $profile->getLink();
					}
				} 
				// RSEvents!Pro
				else if ($profile == 5) {
					...
					$profileURL[$hash] = JRoute::_('index.php?option=com_rseventspro&layout=user&id='.rseventsproHelper::sef($id, $name));
				}
			} else {
				$profileURL[$hash] = '';
			}
	...
	public static function rating($id, $array = false) {
		...
		static $ratingVotes = array();
		static $ratingCount = array();
		
		// Get the rating value
		if (!isset($ratingVotes[$id])) {
			$query->clear()
				->select('CEIL(IFNULL(SUM(value)/COUNT(id),0))')
				->from($db->qn('#__rseventspro_rating'))
				->where($db->qn('ide').' = '.(int) $id);
			...
			$db->setQuery($query);
			$ratingVotes[$id] = $db->loadResult();
		}
		
		// Get the rating count
		if (!isset($ratingCount[$id])) {
			$query->clear()
				->select('COUNT(id)')
				->from($db->qn('#__rseventspro_rating'))
				->where($db->qn('ide').' = '.(int) $id);
			...
			$db->setQuery($query);
			$ratingCount[$id] = $db->loadResult();
		}
		
		if ($array) {
			return array($ratingVotes[$id], $ratingCount[$id]);
		}
		...
		if ($voted) {
			...
			$html[] = '<li><a class="fa fa-times hasTooltip" title="'.JText::_('COM_RSEVENTSPRO_REMOVE_VOTE').'" href="'.rseventsproHelper::route('index.php?option=com_rseventspro&task=rseventspro.deletevote&id='.rseventsproHelper::sef($id, $eventname)).'"></a></li>';
		}
		...
		if (!$voted) {
			...
			$script[] = "\t\t".'initial	: '.(int) $ratingVotes[$id].",\n";
			$script[] = "\t\t".'Itemid	: '.JFactory::getApplication()->input->getInt('Itemid',0).",\n";
			$script[] = "\t\t".'id		: '.(int) $id.",\n";
			...
			JFactory::getDocument()->addCustomTag('<script src="'.JHtml::script('com_rseventspro/jquery.rating.js', array('relative' => true, 'pathOnly' => true, 'version' => 'auto')).'?'.$version.'" type="text/javascript"></script>');
		...
		if ($modal == 0) {
			...
			if ($link == 2) {
				...
				if ($js) {
					...
					if (!$sticky)
						$return .= '<script type="text/javascript">window.top.setTimeout(\''.$redirect.'window.parent.jQuery(".modal .close, .modal .btn-close").click();\',1200);</script>';
					return $return;
				} 
				else return '<a href="javascript:void(0)" onclick="window.parent.jQuery(\'.modal .close, .modal .btn-close\').click();">'.$message.'</a>';
			}
		}
	}
	...
	public static function globalDiscount($id, $total, $tickets, $payment, $multiple = false) {
		...
		if (is_array($id)) {
			$query->where($db->qn('discounttype').' = '.$db->q(2));
		}
	...
	public static function checkticket($id) {
		...
		if ($ticket) {
			...
			if (!isset($events[$ticket->ide])) {
				...
				$db->setQuery($query);
				$events[$ticket->ide] = $db->loadObject();
			}
			
			$event = $events[$ticket->ide];
	...
	public static function getOptions() {
		...
		if ($groups) {
			...
			$hash	= md5(serialize($groups));
			
			static $eventoptions = array();
			
			if (!isset($eventoptions[$hash])) {
				$db			= JFactory::getDbo();
				$query		= $db->getQuery(true);
				...
				$eventoptions[$hash] = $default;
			}
			
			return $eventoptions[$hash];
		}
	...
	public static function getScan() {
		...
		if ($subscriber = $db->loadObject()) {
			...
			$app->triggerEvent('onrsepro_scanTotal', array(array('ids' => $ids,'total' => &$total)));
	...
	public static function getCanSubscribe($id, $skip = false) {
		...
		$query->clear()
			...
			->select($db->qn('form'))->select($db->qn('allday'))->select($db->qn('start'))->select($db->qn('published'))
			->from($db->qn('#__rseventspro_events'))
			->where($db->qn('id').' = '.$db->q($id));
		...
		if ($event->published == 3) {
			return array('status' => false, 'err' => JText::_('COM_RSEVENTSPRO_REGISTRATION_ERROR7'));
		}
	...
	public static function richSnippet($details) {
		...
		$json['eventAttendanceMode'] = 'https://schema.org/MixedEventAttendanceMode';
	...
	public static function canRSVP($id) {
		...
		// Get the event
		$query->clear()
			->select($db->qn('rsvp'))->select($db->qn('rsvp_start'))
			->select($db->qn('rsvp_end'))->select($db->qn('published'))
			->from($db->qn('#__rseventspro_events'))
			->where($db->qn('id').' = '.$db->q($id));
	...
	public static function getSpeakers($id) {
		...
		if (!isset($speakers[$id])) {
			...
			$db->setQuery($query);
			$speakers[$id] = $db->loadObjectList();
		}
		
		if ($speakers[$id]) {
			foreach ($speakers[$id] as $speaker) {
	...
	public static function getSponsors($id) {
		static $sponsors = array();
		...
		if (!isset($sponsors[$id])) {
			...
			$query->clear()
				->select('s.*')
				->from($db->qn('#__rseventspro_sponsors','s'))
				->join('left',$db->qn('#__rseventspro_taxonomy','tx').' ON '.$db->qn('tx.id').' = '.$db->qn('s.id'))
			...
			$db->setQuery($query);
			$sponsors[$id] = $db->loadObjectList();
		}
		
		if ($sponsors[$id]) {
			foreach ($sponsors[$id] as $sponsor) {
				if ($sponsor->url && substr($sponsor->url,0,4) != 'http') {
	...
	public static function timezoneModal() {
		...
		$footer[] = '<button class="btn btn-danger" data-dismiss="modal" data-bs-dismiss="modal" aria-hidden="true">'.JText::_('COM_RSEVENTSPRO_GLOBAL_CANCEL').'</button>';
		...
		$html[] = '<input type="text" class="form-control" disabled value="'.$timezone.'" />';
		$html[] = '</div>';
		$html[] = '</div>';
		$html[] = '<div class="control-group">';
		$html[] = '<div class="control-label">';
	...
	public static function facebookEventsIDs() {
		static $ids = array();
	...
	public static function parseFacebookEvent($event, $pagename, $owners, $from) {
		$log			= array();
		$db				= JFactory::getDbo();
		$query			= $db->getQuery(true);
		$fbids			= rseventsproHelper::facebookEventsIDs();
		$config			= rseventsproHelper::getConfig();
		$expired		= isset($config->facebook_expired) ? $config->facebook_expired : 1;
		$recurring		= isset($config->facebook_recurring) ? $config->facebook_recurring : 1;
		$checkOwnerPage	= isset($config->facebook_check_owner) ? $config->facebook_check_owner : 1;
		$checkOwnerUser	= isset($config->facebook_check_owner_profile) ? $config->facebook_check_owner_profile : 1;
		$idcategory		= $config->facebook_category;
		$idlocation		= $config->facebook_location;
		$events			= array();
		$total			= 0;
		...
		if ($from == 'page') {
			$pagename = JText::sprintf('COM_RSEVENTSPRO_SYNC_LOG_PAGE', $pagename);
		} elseif ($from == 'group') {
			$pagename = JText::sprintf('COM_RSEVENTSPRO_SYNC_LOG_GROUP', $pagename);
		}
		
		$log[$event['id']] = array('name' => $event['name'], 'date' => JFactory::getDate()->toSql(), 'imported' => false, 'message' => '', 'page' => $pagename, 'from' => @$event['owner']['name'], 'eventID' => 0);
		
		// Check if we already added this Facebook event
		if (in_array($event['id'], $fbids)) {
			$log[$event['id']]['message'] = JText::_('COM_RSEVENTSPRO_SYNC_LOG_ERROR_DB');
			rseventsproHelper::saveSyncLog($log, 'facebook');
			
			return false;
		}
		
		// Check the event owner
		if ($checkOwnerUser && $from == 'user' || $checkOwnerPage && $from == 'page') {
			$owner	 = isset($event['owner']) ? $event['owner'] : array();
			$ownerID = !empty($owner) && !empty($owner['id']) ? $owner['id'] : 0;
		...
		if (isset($event['event_times']) && is_array($event['event_times']) && $recurring) {
			foreach($event['event_times'] as $recurringEvent) {
				$log[$recurringEvent['id']] = array('name' => '[REC] '.$event['name'], 'date' => JFactory::getDate()->toSql(), 'imported' => false, 'message' => '', 'page' => $pagename, 'from' => @$event['owner']['name'], 'eventID' => 0);
		...
		// Parse events
		if ($events) {
			// Create a new category if none is selected
			if (empty($idcategory)) {
				$query->clear()
					->insert($db->qn('#__rseventspro_categories'))
					->set($db->qn('name').' = '.$db->q('Facebook events'));
			...
			foreach ($events as $fbevent) {
				$cover		= isset($fbevent['cover']) ? $fbevent['cover'] : array();
				$timezone	= isset($fbevent['timezone']) ? $fbevent['timezone'] : null;
				$image		= '';
				...
				if (isset($fbevent['start_time'])) {
					if ($fbevent['start_time'] instanceof DateTime) {
						$startDate = new DateTime($fbevent['start_time']->format('Y-m-d H:i:s'));
					} else {
						$startDate = new DateTime($fbevent['start_time']);
					}
				} else {
					$startDate = new DateTime();
				}
				...
				if (isset($fbevent['end_time'])) {
					if ($fbevent['end_time'] instanceof DateTime) {
						$endDate = new DateTime($fbevent['end_time']->format('Y-m-d H:i:s'));
					} else {
						$endDate = new DateTime($fbevent['end_time']);
					}
				...
				if (!$expired) {
					if ($now > $endDate) {
						$log[$fbevent['id']]['message'] = JText::_('COM_RSEVENTSPRO_SYNC_LOG_EXPIRED');
						continue;
					}
				}
				...
				$eventDescr			= @$fbevent['description'];
				$locationName		= isset($fbevent['place']['name']) ? $fbevent['place']['name'] : 'Facebook Location';
				$locationStr		= isset($fbevent['place']['location']['street']) ? $fbevent['place']['location']['street'] : '';
				$locationCity		= isset($fbevent['place']['location']['city']) ? $fbevent['place']['location']['city'] : '';
				$locationState		= isset($fbevent['place']['location']['state']) ? $fbevent['place']['location']['state'] : '';
				$locationCountry	= isset($fbevent['place']['location']['country']) ? $fbevent['place']['location']['country'] : '';
				$locationLat		= isset($fbevent['place']['location']['latitude']) ? $fbevent['place']['location']['latitude'] : '';
				$locationLon		= isset($fbevent['place']['location']['longitude']) ? $fbevent['place']['location']['longitude'] : '';
				
				if (empty($idlocation)) {
					$address = $locationStr;
					if (!empty($locationCity))		$address .= ' , '.$locationCity;
					if (!empty($locationState))		$address .= ' , '.$locationState;
					if (!empty($locationCountry))	$address .= ' , '.$locationCountry;
					
					// Check if we already have this location
					$query->clear()
						->select($db->qn('id'))
						->from($db->qn('#__rseventspro_locations'))
						->where($db->qn('name').' = '.$db->q($locationName))
						->where($db->qn('address').' = '.$db->q($address))
						->where($db->qn('coordinates').' = '.$db->q($locationLat.','.$locationLon));
					$db->setQuery($query);
					if (!$idlocation = (int) $db->loadResult()) {
						$query->clear()
							->insert($db->qn('#__rseventspro_locations'))
							->set($db->qn('name').' = '.$db->q($locationName))
							->set($db->qn('address').' = '.$db->q($address))
							->set($db->qn('coordinates').' = '.$db->q($locationLat.','.$locationLon))
							->set($db->qn('published').' = '.$db->q(1));
				...
				$query->clear()
					...
					->set($db->qn('location').' = '.$db->q($idlocation))
					->set($db->qn('owner').' = '.$db->q(JFactory::getUser()->get('id')))
					->set($db->qn('name').' = '.$db->q($eventName))
					->set($db->qn('description').' = '.$db->q($eventDescr))
					->set($db->qn('start').' = '.$db->q($start))
					->set($db->qn('end').' = '.$db->q($end))
					->set($db->qn('allday').' = '.$db->q(0))
					->set($db->qn('options').' = '.$db->q(rseventsproHelper::getDefaultOptions()))
					->set($db->qn('completed').' = '.$db->q(1))
					->set($db->qn('published').' = '.$db->q(1));
				
				if ($timezone) {
					$query->set($db->qn('timezone').' = '.$db->q($timezone));
				}
				...
				$log[$eventID]['imported'] = true;
				$log[$eventID]['eventID'] = $idevent;
	...
	public static function exportUnsubscribersCSV($query) {
		$db		= JFactory::getDbo();
		$id		= JFactory::getApplication()->input->getInt('id', 0);
		$csv	= '';
		...
		if (rseventsproHelper::getConfig('export_headers')) {
			$csv .= '"'.rseventsproHelper::fixValue(JText::_('COM_RSEVENTSPRO_UNSUB_ID')).'",';
			$csv .= '"'.rseventsproHelper::fixValue(JText::_('COM_RSEVENTSPRO_UNSUB_NAME')).'",';
			$csv .= '"'.rseventsproHelper::fixValue(JText::_('COM_RSEVENTSPRO_UNSUB_EMAIL')).'",';
			$csv .= '"'.rseventsproHelper::fixValue(JText::_('COM_RSEVENTSPRO_UNSUB_DATE')).'",';
			$csv .= "\n";
		}
		
		if (!empty($users)) {
			foreach ($users as $user) {				 
				$csv .= '"'.$db->escape(rseventsproHelper::fixValue($user->id)).'",';
				$csv .= '"'.$db->escape(rseventsproHelper::fixValue($user->name)).'",';
				$csv .= '"'.$db->escape(rseventsproHelper::fixValue($user->email)).'",';
				$csv .= '"'.$db->escape(rseventsproHelper::fixValue(rseventsproHelper::showdate($user->date,'Y-m-d H:i:s'))).'",';
				$csv .= "\n";
			}
		}
		
		$db->setQuery('SELECT '.$db->qn('name').' FROM '.$db->qn('#__rseventspro_events').' WHERE '.$db->qn('id').' = '.$db->q($id));
		$name = $db->loadResult();
		
		$file = htmlentities(JText::sprintf('COM_RSEVENTSPRO_UNSUB_EVENT',$name), ENT_COMPAT, 'UTF-8');
		header('Content-type: text/csv; charset=UTF-8');
	...
	public static function hasUnsubscribers($id) {
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
	...
	public static function isInWaitingList() {
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
		$id		= JFactory::getApplication()->input->getInt('id', 0);
		$user	= JFactory::getUser();
	...
	public static function hasInvoice($id) {
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
		...
		if ($eid) {
			static $hasinvoice = array();
			
			if (!isset($hasinvoice[$eid])) {
				$query->clear()
					->select($db->qn('invoice'))
					->from($db->qn('#__rseventspro_events'))
					->where($db->qn('id').' = '.$db->q($eid));
				$db->setQuery($query);
				$hasinvoice[$eid] = (bool) $db->loadResult();
			}
			
			return $hasinvoice[$eid];
			...
			$query->clear()
				...
				->where($db->qn('ids').' = '.$db->q($id));
			$db->setQuery($query);
			if ($events = $db->loadResult()) {
				$query->clear()
					->select('MAX('.$db->qn('invoice').')')
					->from($db->qn('#__rseventspro_events'))
					->where($db->qn('id').' IN ('.$events.')');
				$db->setQuery($query);
				return (bool) $db->loadResult();
			}
	...
	public static function gRecaptcha($id) {
		$doc	= JFactory::getDocument();
		$config = rseventsproHelper::getConfig();
		...
		$params = array(
			...
			'type'	  => $config->recaptcha_type
		);
		
		$params = json_encode($params);
		
		$script[] = "<script type=\"text/javascript\">";
		$script[] = "\tRSEventsProReCAPTCHAv2.loaders.push(function(){";
		$script[] = "\t\tif (typeof RSEventsProReCAPTCHAv2.containers['".$id."'] === 'undefined') {";
		$script[] = "\t\t\tvar id = grecaptcha.render('".$id."', {$params});";
		$script[] = "\t\t\tRSEventsProReCAPTCHAv2.containers['".$id."'] = id;";
		$script[] = "\t\t}";
		$script[] = "\t});";
		$script[] = "</script>";
	...
	public static function hCaptcha($id) {
		$doc	= JFactory::getDocument();
		$config = rseventsproHelper::getConfig();
		
		$lang = $config->hcaptcha_lang != 'auto' ? '&amp;hl='.JFactory::getLanguage()->getTag() : '';
		
		$doc->addCustomTag('<script src="https://hcaptcha.com/1/api.js?render=explicit'.$lang.'" async defer></script>');
		
		$script		= array();
		$onsubmit	= array();
		$theme		= strtolower($config->hcaptcha_theme);
		$size		= strtolower($config->hcaptcha_size);
		...
		if ($size == 'invisible') {
			$params['callback'] = 'RSEventsProInvisiblehCaptchaCallback';
			
			$onsubmit[] = "rseAddEvent(document.getElementById('".$id."Form'), 'submit', function(evt){";
			$onsubmit[] = "\t\t\t\tevt.preventDefault();";
			$onsubmit[] = "\t\t\t\thcaptcha.execute(id);";
			$onsubmit[] = "\t\t\t});";
			$onsubmit[] = "\t\t\tdocument.getElementById('".$id."Btn').onclick = null;";
		}
		...
		if ($size == 'invisible') {
			$script[] = "\tfunction RSEventsProInvisiblehCaptchaCallback() {";
			...
			if ($id == 'subscribe') {
				...
				$function = 'RSEProCart.checkout';
			}
			
			if ($function) {
				$script[] = "\t\tif (".$function."()) {";
				$script[] = "\t\t\tdocument.getElementById('".$id."Form').submit();";
				$script[] = "\t\t}";
			}
		...
		$script[] = "\tRSEventsProhCaptcha.loaders.push(function(){";
		$script[] = "\t\tif (typeof RSEventsProhCaptcha.containers['".$id."'] === 'undefined') {";
		$script[] = "\t\t\tvar id = hcaptcha.render('h-captcha-".$id."', {$params});";
		$script[] = "\t\t\tRSEventsProhCaptcha.containers['".$id."'] = id;";
		$script[] = "\t\t\t".implode("\n", $onsubmit)."";
		$script[] = "\t\t}";
		$script[] = "\t});";
	...
	public static function getPurchasedTickets($id) {
		static $purchasedTickets = array();
		
		$id = (int) $id;

		if (!isset($purchasedTickets[$id])) {
			$db		= JFactory::getDbo();
			$query	= $db->getQuery(true);
			...
			$purchasedTickets[$id] = isset($purchased) ? $purchased : 0;
			
		}
		
		return $purchasedTickets[$id];
	}
	
	public static function addUserEventCount($ide, $owner = null) {
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
		$userID	= is_null($owner) ? JFactory::getUser()->get('id') : $owner;
		
		$query->clear()
			->select('COUNT('.$db->qn('ide').')')
			->from($db->qn('#__rseventspro_taxonomy'))
		...
		if (!$db->loadResult()) {
			$query->clear()
				->insert($db->qn('#__rseventspro_taxonomy'))
				->set($db->qn('type').' = '.$db->q('userCount'))
				->set($db->qn('ide').' = '.$db->q($ide))
				->set($db->qn('id').' = '.$db->q($userID));
			$db->setQuery($query);
			$db->execute();
		}
	}
	
	public static function getUserEventCount($uid = null) {
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
		$userID	= is_null($uid) ? JFactory::getUser()->get('id') : $uid;
	...
	public static function canUnsubscribe($id, $check = false) {
		...
		if ($check) {
			...
			if (empty($permissions['can_unsubscribe'])) {
				return false;
			}
		}
		
		$query->clear()
			->select($db->qn('unsubscribe_date'))
			->from($db->qn('#__rseventspro_events'))
			->where($db->qn('id').' = '.$id);
		
		$db->setQuery($query);
		$unsubscribe_date = $db->loadResult();
		
		if (!empty($unsubscribe_date) && $unsubscribe_date != $db->getNullDate()) {
