...
class RseventsproRouter extends JComponentRouterView {
	...
	public function build(&$query) {
			...
			switch ($query['view']) {
				...
				case 'rseventspro':
					...
					switch($query['layout']) {
						...
						case 'waiting':
							$segments[] = JText::_('COM_RSEVENTSPRO_WAITING_SEF');
						...
						case 'waitinglist':
							$segments[] = JText::_('COM_RSEVENTSPRO_WAITINGLIST_SEF');
							
							if (isset($query['id']))
								$segments[] = $query['id'];
						break;
						
						case 'editwaitinglist':
							$segments[] = JText::_('COM_RSEVENTSPRO_WAITINGLIST_EDIT_SEF');
						...
						case 'unsubscribers':
							$segments[] = JText::_('COM_RSEVENTSPRO_UNSUBSCRIBERS_SEF');
		...
		if (isset($query['task'])) {
			switch ($query['task']) {
				...
				case 'rseventspro.approvewaiting':
					$segments[] = JText::_('COM_RSEVENTSPRO_WAITINGLIST_APPROVE_SEF');
					
					if (isset($query['id']))
						$segments[] = $query['id'];
				break;
				
				case 'rseventspro.removewaiting':
					$segments[] = JText::_('COM_RSEVENTSPRO_WAITINGLIST_REMOVE_SEF');
				...
				case 'claim':
					$segments[] = JText::_('COM_RSEVENTSPRO_CLAIM_TICKET_SEF');
					
					if (isset($query['hash']))
						$segments[] = $query['hash'];
				break;
				
				case 'rseventspro.deletevote':
					$segments[] = JText::_('COM_RSEVENTSPRO_DELETE_VOTE_SEF');
					
					if (isset($query['id']))
						$segments[] = $query['id'];
				break;
				
				case 'rseventspro.exportunsubscribers':
					$segments[] = JText::_('COM_RSEVENTSPRO_EXPORT_UNSUBSCRIBERS_SEF');
					
					if (isset($query['id']))
						$segments[] = $query['id'];
				break;
				
				case 'rseventspro.removeunsubscriber':
					$segments[] = JText::_('COM_RSEVENTSPRO_REMOVE_UNSUBSCRIBER_SEF');
				...
				case 'rseventspro.cancelevent':
					$segments[] = JText::_('COM_RSEVENTSPRO_CANCEL_SEF');
					
					if (isset($query['id']))
						$segments[] = $query['id'];
				break;
				
				case 'rseventspro.invoice':
					$segments[] = JText::_('COM_RSEVENTSPRO_INVOICE_SEF');
	...
	public function parse(&$segments) {
		...
		switch ($segments[0]) {
			...
			case JText::_('COM_RSEVENTSPRO_WAITING_SEF'):
				$query['view']		= 'rseventspro';
			...
			case JText::_('COM_RSEVENTSPRO_WAITINGLIST_SEF'):
				$query['view']		= 'rseventspro';
				$query['layout']	= 'waitinglist';
				$query['id']		= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_WAITINGLIST_EDIT_SEF'):
				$query['view']		= 'rseventspro';
				$query['layout']	= 'editwaitinglist';
				$query['id']		= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_WAITINGLIST_APPROVE_SEF'):
				$query['task']			= 'rseventspro.approvewaiting';
				$query['id']			= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_WAITINGLIST_REMOVE_SEF'):
				$query['task']			= 'rseventspro.removewaiting';
				$query['id']			= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			...
			case JText::_('COM_RSEVENTSPRO_CLAIM_TICKET_SEF'):
				$query['task']			= 'claim';
				$query['hash']			= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_DELETE_VOTE_SEF'):
				$query['task']			= 'rseventspro.deletevote';
				$query['id']			= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_UNSUBSCRIBERS_SEF'):
				$query['view']		= 'rseventspro';
				$query['layout']	= 'unsubscribers';
				$query['id']		= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_EXPORT_UNSUBSCRIBERS_SEF'):
				$query['task']			= 'rseventspro.exportunsubscribers';
				$query['id']			= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_REMOVE_UNSUBSCRIBER_SEF'):
				$query['task']			= 'rseventspro.removeunsubscriber';
				$query['id']			= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
				$query['ide']			= isset($segments[2]) ? str_replace(':','-',$segments[2]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_CANCEL_SEF'):
				$query['task']			= 'rseventspro.cancelevent';
				$query['id']			= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
			break;
			
			case JText::_('COM_RSEVENTSPRO_INVOICE_SEF'):
				$query['task']			= 'rseventspro.invoice';
				$query['id']			= isset($segments[1]) ? str_replace(':','-',$segments[1]) : null;
				$query['ide']			= isset($segments[2]) ? str_replace(':','-',$segments[2]) : null;
			break;
		}
