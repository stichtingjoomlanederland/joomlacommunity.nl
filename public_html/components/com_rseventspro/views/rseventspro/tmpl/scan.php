...
<form action="<?php echo rseventsproHelper::route('index.php?option=com_rseventspro&layout=scan&id='.rseventsproHelper::sef($this->event->id,$this->event->name)); ?>" method="post" id="adminForm" name="adminForm" class="form-horizontal rsepro-horizontal">
	
	<fieldset class="options-form">
		<legend><?php echo JText::_('COM_RSEVENTSPRO_SCAN_TITLE'); ?></legend>
		<?php echo RSEventsproAdapterGrid::renderField('<label for="ticket">'.JText::_('COM_RSEVENTSPRO_SCAN_LABEL').'</label>', '<input class="form-control" type="text" name="ticket" id="ticket" tabindex="1" />'); ?>
	</fieldset>
