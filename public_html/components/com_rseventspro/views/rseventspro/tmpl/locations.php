		...
		<div class="<?php echo RSEventsproAdapterGrid::card(); ?>">
			<div class="card-body">
				...
				<div class="rs_location_info">
					<?php if ($this->params->get('address', 1) && !empty($location->address)) { ?><i class="fa fa-map-marker fa-fw"></i> <?php echo $location->address; ?><br><?php } ?>
					<?php if ($this->params->get('url', 1) && !empty($location->url)) { ?><i class="fa fa-globe fa-fw"></i> <a href="<?php echo $location->url; ?>" target="_blank"><?php echo $location->url; ?></a><?php } ?>
				</div>
				<?php } ?>
...
<?php if ($this->total > $count) { ?>
	<p id="rsepro_number_events"><?php echo JText::sprintf('COM_RSEVENTSPRO_SHOWING_LOCATIONS','<span>'.$count.'</span>',$this->total); ?></p>
