...
<div class="btn-group btn-group-sm pull-right reply-options">
		
		<?php if( $access->canReply() ){ ?>
		<a href="javascript:void(0);" class="btn btn-default quotePost" rel="ed-tooltip" data-original-title="<?php echo JText::_( 'COM_EASYDISCUSS_QUOTE' , true );?>">
			<span class="icon jc-quote"></span>
		...
		<a class="btn btn-default" href="<?php echo DiscussRouter::getPostRoute( $post->parent_id ) . '#' . JText::_('COM_EASYDISCUSS_REPLY_PERMALINK') . '-' . $post->id;?>" title="<?php echo JText::_('COM_EASYDISCUSS_REPLY_PERMALINK_TO'); ?>" rel="ed-tooltip" data-original-title="<?php echo JText::_( 'COM_EASYDISCUSS_REPLY_PERMALINK_TO' , true );?>">
			<span class="icon jc-link"></span>
		</a>


		<?php if( $system->config->get( 'main_report' ) && DiscussHelper::getHelper( 'ACL' )->allowed( 'send_report' ) && ($my->id != $owner) ){ ?>
		<a onclick="discuss.reports.add('<?php echo $post->id;?>');" href="javascript:void(0);" class="btn btn-default" rel="ed-tooltip" data-original-title="<?php echo JText::_( 'COM_EASYDISCUSS_REPORT_THIS' , true );?>">
			<span class="icon jc-report"></span>
		</a>
		<?php } ?>
		...
		<a href="<?php echo DiscussRouter::getPrintRoute( $post->id );?>; ?>"
			...
			class="btn btn-default" rel="ed-tooltip" data-original-title="<?php echo JText::_( 'COM_EASYDISCUSS_PRINT' , true );?>">
		...
		<div class="btn-group btn-group-sm">
			...
			<a class="btn btn-default dropdown-toggle" data-foundry-toggle="dropdown">
				<span class="icon jc-moderate"></span>
				<?php echo JText::_( 'COM_EASYDISCUSS_MODERATION_TOOLS' ); ?>&nbsp;<span class="caret"></span>
			</a>
			...
			<ul class="dropdown-menu reset-ul pull-right">
				...
				<?php if( $post->isReply() && $access->canReply() && $access->canBranch() ){ ?>
					<li>
						<a href="javascript:void(0);" onclick="discuss.post.branch('<?php echo $post->id;?>')">
