		...
		if (!empty($avatar)) {
			...
			switch ($avatar) {
				...
				case 'easydiscuss':
					...
					// Use absolute path to prevent loading the default com_easydiscuss avatar
					if (file_exists(JPATH_SITE.'/'.$profile->config->get('main_avatarpath').$profile->avatar)) {
						$html .= '<img src="'.$profile->getAvatar().'" alt="EasyDiscuss Avatar" class="ed_avatar" height="64" />';
					} else {
						$html .= '<img src="'.JURI::root().'components/com_rseventspro/assets/images/user.png" alt="K2 Avatar" class="rs_avatar" width="64" height="64" />';
					}
				break;
