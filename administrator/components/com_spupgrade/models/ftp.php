...
class SPUpgradeClientFtp {
    ...
    protected function _passive() {
        ...
        if (!$this->_dataconn) {
            JLog::add(
                    JText::sprintf('JLIB_CLIENT_ERROR_JFTP_PASSIVE_CONNECT', $this->_pasv['ip'], $this->_pasv['port'], $errno, $err), JLog::WARNING, 'jerror'
            );
...
class SPUpgradeModelFTP extends JModelLegacy {
            ...
            if ($item->type == 0) {
                ...
                if ($replace == 1 && JFile::exists($localPath . '/' . $item->name)) {
                    ...
                    if (!$ftp->get($localPath . '/' . $item->name, $item->name)) {
                        ...
                        jexit('<p><font color="red">' . JText::sprintf('COM_SPUPGRADE_COPY_FILE_ERROR', $localPath . '/' . $item->name) . '</font></p>');
                        continue;
                    } else {
                ...
                if ($item->name != '..') {
                    ...
                    if (!file_exists($localPath . '/' . $item->name)) {
                        if (!JFolder::create($localPath . '/' . $item->name)) {
                            ...
                            jexit('<p><font color="red">' . JText::sprintf('COM_SPUPGRADE_CREATE_FOLDER_ERROR', $localPath . '/' . $item->name) . '</font></p>');
                            continue;
                        } else {
    ...
    protected function ftp_connect($credentials = null) {
        ...
        $ftp = new SPUpgradeClientFtp(Array());
