...
<form action="<?php echo JRoute::_('index.php?option=com_rsfiles&view=group&layout=edit&IdGroup='.(int) $this->item->IdGroup); ?>" method="post" name="adminForm" id="adminForm" autocomplete="off" class="form-validate form-horizontal">
	<div class="row-fluid">
		<div class="span6 rsspan6 rslft">
			<?php $extra = '<span class="rs_extra"><a class="modal" rel="{handler: \'iframe\'}" href="'.JRoute::_('index.php?option=com_users&view=users&layout=modal&tmpl=component&field=jform_jusers'.(!empty($this->excludes) ? ('&excluded=' . base64_encode(json_encode($this->excludes))) : '')).'">'.JText::_('COM_RSFILES_GROUP_ADD_USERS').'</a>'; ?>
