...
<?php $events = modRseventsProCategories::getCount($item->id); ?>
	...
	<?php $levelup = $item->level - $startLevel - 1; ?>
		...
		<?php if ($remove) { ?>
			...
			<?php } ?>
		<?php } else { ?>
			<a class="list-group-item" <?php echo $open; ?> href="<?php echo rseventsproHelper::route('index.php?option=com_rseventspro&category='.rseventsproHelper::sef($item->id,$item->title),true,$itemid); ?>">
				<?php echo(str_replace('Joomla! gebruikersgroep', '', $item->title));?> <?php if ($counter && $events) { ?> (<?php echo JText::plural('MOD_RSEVENTSPRO_CATEGORIES_EVENTS_COUNT',$events); ?>) <?php } ?>
			</a>
		<?php } ?>
